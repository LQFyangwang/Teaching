...
public class ProductServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (method.equals("all")) {
			...
			try {
				int page = Integer.valueOf(method);
				method = WebUtil.getUriMethod(req, 2);
				if (method.equals("pager")) {
					queryByPager1(req, resp);
				}
	...
	private void queryByPager1(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		int pageNo = Integer.valueOf(WebUtil.getUriMethod(req));
		final int pageSize = 5;
		int total = productService.count();
		int totalPage = (total % pageSize) == 0 ? total / pageSize : (total / pageSize + 1);
		if (pageNo <= 0) {
		...
		req.setAttribute("currentPage", pageNo);
		req.setAttribute("totalPage", totalPage);
		req.getRequestDispatcher("/products_pager1.jsp").forward(req, resp);
	}
