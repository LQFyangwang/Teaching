...
<script>
	...
	var jsonStr = '{"stu.stuId":"_stuid","stu.emp.empId":"_empid","stu.name":"_name","stu.phone":"_phone","stu.qq":"_qq","stu.wechat":"_wechat","stu.school":"_school","stu.birthday":"_birthday","stu.gender":"_gender","stu.nation":"_nation","stu.empId":"_empid","stu.emp.name":"_emp_name","stu.status":"_status","stu.stuStatus":"_stustatus"}';
					...
					if (method == "add") {
						...
						if (row) {
							$.messager.prompt('缴费提示', '请缴纳学费，预定学生基础费用是￥500', function(r){
								if (r) {
									if (!isNaN(r)) {
										$.post("<%=path %>/stu/updateStuStatus?flag=2&r=" + r + "&stuId=" + row.stuId,
												function(data) {
													if (data.result.result == "success") {
							...
							$.messager.alert("提示","请选择一个需要转为预定的学生", "info");
						}
	...
	function showAddStuWin() {
		authority('com.ht.action.StuAction.save?flag=1', "", "add");
	}
	...
	function showEditStuWin() {
		var row = $('#list').datagrid("getSelected");
		if (row) {
			var jsonStr1 = jsonStr.replace("_stuid", row.stuId).replace("_empid", row.emp.empId).replace("_name", row.name).replace("_phone", row.phone).replace("_qq", getValue(row.qq)).replace("_wechat", getValue(row.wechat)).replace("_school", getValue(row.school)).replace("_birthday", row.birthday).replace("_gender", row.gender).replace("_nation", getValue(row.nation)).replace("_status", row.status).replace("_stustatus", row.stuStatus);
			authority('com.ht.action.StuAction.update?flag=1', jsonStr1, "edit");
		} else {
	...
	function reserve(){
		authority('com.ht.action.StuAction.updateStuStatus', "", "reserve");
	}
	...
	function queryByStuName(name, value) {
		$.get("<%=path %>/stu/fuzzySearch1?name=" + name + "&value=" + value, 
				function(data) {
					if (data.result.result == "success") {
...
<table id="list" class="easyui-datagrid" 
			...
			collapsible:true,
			url:'<%=path %>/stu/intentionStuByPager',
			method:'get',
			rownumbers:true,
			autoRowHeight:true,
			pagination:true,
	...
	<thead>
			<tr>
				...
				<th data-options="field:'emp',width:80" formatter="formatEmpName">招生老师</th>
...
<div id="tb" style="padding:5px;height:auto">
	<div style="margin-bottom:5px">
		<a href="javascript:;" onclick="showAddStuWin('${sessionScope.emp.name }');" class="easyui-linkbutton" iconCls="icon-add">添加</a>
		<a href="javascript:;" onclick="showEditStuWin();" class="easyui-linkbutton" iconCls="icon-edit">编辑</a>
		<a href="javascript:;" onclick="reserve();" class="easyui-linkbutton" iconCls="icon-add">转为预定学生</a>
...
<div class="easyui-window" id="addWin" title="添加学生" data-options="closed:true" style="width:600px;height:300px;">
	<form id="addForm">
		<table>
			<tr>
				<td style="padding: 5px;">招生老师</td>
				<td><input class="easyui-textbox" id="addEmpName" name="stu.emp.name" data-options="editable:false" /></td>
...
<div class="easyui-window" id="editWin" title="修改学生" data-options="closed:true" style="width:600px;height:300px;">
		<form id="editForm">
			<input type="hidden" id="stuid" name="stu.stuId" />
