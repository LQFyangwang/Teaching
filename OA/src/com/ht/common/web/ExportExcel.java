...
public final class ExportExcel {
 ...
 /***
   ...
   *        sheet名称和表头值
   */
  public static void excelExport(List<?> dataList, Map<String, String> titleMap, String sheetName, String tableName, String fileName) {
    // 初始化workbook
    ...
    try {
      ...
      OutputStream out = new FileOutputStream(fileName + tableName + ".xls");
      workbook.write(out);
      out.close();
    }
    catch (Exception e) {
...
/***
  ...
  private static void createDateHeadRow(Map<String, String> titleMap) {
    ...
    dateCell.setCellValue("统计时间:" + new SimpleDateFormat("yyyy年MM月dd日").format(new Date()));
  }
