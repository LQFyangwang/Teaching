...
<script>
		...
		if (rows.length > 0) {
			...
			if (ids != "") {
				$.messager.confirm("提示", "确定删除吗？", function (r) {
					if (r) { // 点击了确定按键
						$.get("<%=path %>/product/deleteByIds?ids=" + ids, function (data) {
	...
	function removeProduct1(index) {
		...
		if (row) {
				$.messager.confirm("提示", "确定删除吗？", function (r) {
					if (r) { // 点击了确定按键
						$.get("<%=path %>/product/deleteByIds?ids=" + row.id, function (data) {
							$("#list").datagrid("reload");
						}, "json");
					}
				})
	...
	function formatterOpt(value, row, index) {
		return "<a href='javascript:;' onclick='showEditProductWin1(" + index + ");'>编辑</a>" +
